...
// The 4 things worth a thumb reaching for right away on a phone — everything
// else (Revenue, Showroom, AI Assistant, and the owner-only back-office
// pages) is one tap further, behind "More", rather than filling the whole
// screen with a long nav list on first open.
const PRIMARY_IDS = ["overview", "companies", "pipeline", "notes"];

function SearchBox({ query, setQuery, open, setOpen, results, selectResult }) {
  return (
...
function ProfileFooter({ profile, showsRegion, signOut }) {
...
export default function Sidebar({ page, setPage, setSelectedCompanyId, companies = [], notes = [] }) {
  ...
  const primaryItems = items.filter((it) => PRIMARY_IDS.includes(it.id));
  const moreItems = items.filter((it) => !PRIMARY_IDS.includes(it.id));
  ...
  return (
    <>
      ...
      <div
        className="hidden md:flex md:flex-col md:justify-between md:shrink-0"
      ...
      >
        ...
        <ProfileFooter profile={profile} showsRegion={showsRegion} signOut={signOut} />
      </div>

      {/* Mobile: a slim top bar, hamburger opens a full-screen drawer. */}
      <div
        className="flex md:hidden items-center justify-between px-4 shrink-0"
        style={{ height: 52, background: T.surface, borderBottom: `1px solid ${T.border}` }}
      >
        <img src="/lemo-logo.png" alt="Lemo" style={{ height: 22, width: "auto" }} />
        <button onClick={() => setMobileOpen(true)} style={{ color: T.text }} aria-label="Open menu">
          <Menu size={22} />
        </button>
      </div>

      {mobileOpen && (
        <div className="flex md:hidden fixed inset-0 z-50 flex-col" style={{ background: T.surface }}>
          <div className="flex items-center justify-between px-4" style={{ height: 52, borderBottom: `1px solid ${T.border}` }}>
            <img src="/lemo-logo.png" alt="Lemo" style={{ height: 22, width: "auto" }} />
            <button onClick={() => setMobileOpen(false)} style={{ color: T.text }} aria-label="Close menu">
              <X size={22} />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto">
            ...
            {showMore && <NavList items={moreItems} page={page} onSelect={goTo} />}
          </div>
          {isBdConsultant && (
            <div className="px-4 py-2.5 flex items-start gap-1.5" style={{ borderTop: `1px solid ${T.borderSoft}` }}>
              <ShieldAlert size={12} style={{ color: T.red, marginTop: 1, flexShrink: 0 }} />
