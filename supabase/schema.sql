...
-- Auto-creates the Pre-Install Checklist task the moment an owner (or that
-- region's Strategic Partner) confirms a company's review — instead of a
-- BD consultant having to remember to create an Install-type task
-- themselves. The task itself immediately surfaces as a "Pre-Install
-- Checklist" High Priority Action (helpers.js flags any type='install'
-- task without a completed/bypassed checklist) — no separate HPA wiring
-- needed, just the task existing is enough. Only fires on the true ->
-- false transition (the actual "confirm review" action) — a company that
-- never went through pending_review (owner/geo_partner-created) doesn't
-- get one seeded, since there's no "approval" moment for it to hang off of.
...
-- ============== showroom_bookings ==============
-- Booking calendar for the (single, physical) showroom, so two BD
-- consultants — personal email logins, no shared calendar account — can't
-- accidentally book the same chair-demo slot. No room/location column
-- since there's only one showroom, so the exclusion constraint below is a
-- pure time-range overlap check, no extension (btree_gist) needed since
-- there's no scalar column to pair it with.
--
-- Visible/bookable by any internal role (owner, geo_partner,
-- bd_consultant) — it's just a shared physical resource, not sensitive
-- data. Only the person who booked a slot (or an owner) can edit/cancel it.
...
-- Dual-approval "checks and balances" for the most sensitive Master Admin
-- actions — a single Master Admin can no longer unilaterally turn the
-- site off for everyone, delete a person's account, or invite a new
-- Owner/Strategic Partner. A second, DIFFERENT Master Admin has to
-- approve it first.
--
-- Turning maintenance mode back OFF (restoring access) is NOT gated —
-- that's recovery, not risk, and gating it too would mean a single absent
-- second approver could leave the site down. Inviting a Consultant or
-- Partner isn't gated either — only Owner/Strategic Partner invites (role
-- escalation) and any account deletion need a second sign-off.
--
