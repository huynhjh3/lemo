-- 022: "Submit for Installation" on a pre-install checklist — a separate,
-- later step than marking the checklist complete (migration 021).
-- Completing a checklist just means it's fully filled out; submitting it
-- is the BD consultant explicitly turning it into a work order for Owners
-- (surfaced as a High Priority Action below). Same reset rule as
-- completed_at: any further edit to the checklist (upsertPreInstallChecklist)
-- clears this back to null too, so a stale work order can't survive a
-- changed detail — the consultant has to re-complete and re-submit.
--
-- No new RLS needed: submitting is just another UPDATE on a row the caller
-- could already update (pre_install_checklists_update, migration 021).

alter table pre_install_checklists
  add column submitted_for_install_at timestamptz,
  add column submitted_by uuid references profiles(id) on delete set null;

-- ============== log_activity_audit(): add the submitted-for-install case ==============
