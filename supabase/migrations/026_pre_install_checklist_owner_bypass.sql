-- 026: Owner-only bypass for pre-install checklists — some companies are
-- already installed (predate this feature, or an Owner just doesn't need
-- the logistics captured), and shouldn't need 12 required fields filled in
-- retroactively just to clear the checklist. Bypassing works even when no
-- checklist row exists yet for that outlet (upsert, same as
-- upsertPreInstallChecklist) — it doesn't require completing anything
-- first, unlike Mark Complete.
--
-- No new RLS needed: this is just another upsert/update on a row the
-- caller could already write (pre_install_checklists_insert/update,
-- migration 021) — the UI gates the button to the owner role, same as
-- "Approve for Installation" (migration 024).

alter table pre_install_checklists
  add column bypassed_at timestamptz,
  add column bypassed_by uuid references profiles(id) on delete set null;

-- ============== log_activity_audit(): add the bypassed case ==============
